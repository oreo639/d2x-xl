...
void CTexture::GenerateRenderBuffer ()
{
	...
	uint xOffset = (width - Width ()) / 2;
	uint yOffset = (height - Height ()) / 2;
...
if (bOk) {
	...
	if (bScale) {
		...
		if (actualWidth <= (int)Width () || actualHeight <= (int)Height ()) {
			...
			if (bOk) bOk = 0 < scaledTexture.Shrink (factor, factor);
...
inline bool CTexture::IsAnimated (int nVersion) const
{
