...
INT8 CDiagTool::FindMatCen (INT16 nSegment, INT16* refList)
{
...
for (h = i = 0; i < j; i++, segP++) {
	...
	n = segP->matcen_num;
	if ((n < 0) || (n >= nMatCens))
		n = segP->matcen_num = INT8 (FindMatCen (i));
...
CleanupMatCens (segList, refList, matCenP, nMatCens);
if (!bOk) {
