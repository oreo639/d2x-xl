...
if (theMine->IsWall ()) {
	...
	if (false && m_bShowChildren) {
		...
		for (nChildLine = 0; nChildLine < 4; nChildLine++) {
			...
			nChild = segP->GetChild (nChildSide);
			if (nChild > -1) {
...
DLE.SetModified (TRUE);
DLE.LockUndo ();
theMine->current.Segment ()->SetUV (theMine->Current ()->nSide, 0, 0);
m_alignX = 0;
m_alignY = 0;
m_alignAngle = 0;
Rot2nd (0);
...
{	
	CSegment *segP;
	short nSegment, nSide, nWalls = theMine->MineInfo ().walls.count;
...
{
// set all segment sides as not aligned yet
	CSegment	*currSeg = theMine->current.Segment (),
