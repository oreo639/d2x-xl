/* =============================================================
   16 and 32 bit CCITT CRC Routines
   implemented by David M. Howard from various 
   public domain reference sources. Freely distributable.
   There is no warranty for this code, and it is not guaranteed to work. 
   HOW TO USE TO GENERATE FILE CRC   crc = 0;
   for each character 'ch' in the file      crc = crc16(crc,ch);
   printf("%04x",crc);   HOW TO USE TO TRANSMIT A BLOCK WITH APPENDED CRC
   crc = 0;   for each outgoing character 'ch'       transmit ch
      crc = crc16(crc,ch);
   transmit lsb of crc (important, send in order lsb first..msb)
   transmit msb of crc   HOW TO RECEIVE A BLOCK WITH APPENDED CRC && CHECK IT
   crc = 0   for each incoming character 'ch' including crc bytes
      ch = receive      crc = crc16(crc,ch)   if (crc == 0)       crc was OK
   else       crc check failed   
   make sure 'crc' variable is the proper type by the prototype
===================================================*/
