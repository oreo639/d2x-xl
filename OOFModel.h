...
class CFaceVert {
	...
	public:
		int Read (CFileManager& cf, int bFlipV);
	};
...
class CFace {
	...
	public:
		int Read (CFileManager& cf, CSubModel *pso, CFaceVert *pfv, int bFlipV);
...
class CFaceList {
	public:
		int					m_nFaces;
		CDynamicArray<CFace>		m_list;
		CDynamicArray<CFaceVert>	m_vertices;
...
class CSpecialPoint {
	...
	public:
		...
		void Init (void);
		void Destroy (void);
		int Read (CFileManager& cf);

};

class CSpecialList : public CDynamicArray<CSpecialPoint> {
	public:
		int Read (CFileManager& cf);
	};
...
class CPoint {
	...
	public:
		int Read (CFileManager& cf, int bParent);
	};

class CPointList : public CDynamicArray<CPoint> {
	public:
		int Read (CFileManager& cf, int bParent, int nSize);
	};
...
class CAttachList : public CDynamicArray<CAttachPoint> {
	public:
		int Read (CFileManager& cf);
		int ReadNormals (CFileManager& cf);
	};

class CBattery {
	public:
		int				m_nVerts;
		CDynamicArray<int>		m_vertIndex;
		int				m_nTurrets;
		CDynamicArray<int>		m_turretIndex;
...
class CArmament : public CDynamicArray<CBattery> {
	public:
		int Read (CFileManager& cf);
	};
...
class CFrameInfo {
	...
	public:
		CFrameInfo () { Init (); }
		void Init (void);
		int Read (CFileManager& cf, CModel* po, int bTimed);
};
...
class CPosFrame {
	...
	public:
		int Read (CFileManager& cf, int bTimed);
...
class CAnim : public CFrameInfo {
	public:
		int				m_nTicks;
		CDynamicArray<ubyte>	m_remapTicks;
...
class CPosAnim : public CAnim {
	public:
		CDynamicArray<CPosFrame>	m_frames;
...
class CRotAnim : public CAnim {
	public:
		CDynamicArray<CRotFrame>	m_frames;
	...
	private:
		void BuildAngleMatrix (CDoubleMatrix *pm, int a, CFloatVector *pAxis);
	};
...
class CSubModel {
	public:
		...
		CAngleVector				m_aMod;
		CDoubleMatrix				m_mMod;				// The angles from parent.  Stuffed by model_set_instance
		CFloatVector				m_vMod;
	...
	public:
		...
		void Init (void);
		void Destroy (void);
		int Read (CFileManager& cf, CModel* po, int bFlipV);
...
class CModel {
	...
	private:
		int ReadInfo (CFileManager& cf);
		int ReadTextures (CFileManager& cf);
